...
def test_transactions_by_user_choice_empty_list() -> None:
    result = transactions_by_user_choice("продукты", [])
    assert result == []


def test_transactions_by_user_choice_description(excel_data: list) -> None:
    result = transactions_by_user_choice("продуктов", excel_data)
    assert len(result) == 1
    assert result[0]["Описание"] == "Покупка продуктов"


def test_transactions_by_user_choice_category(excel_data: list) -> None:
    result = transactions_by_user_choice("одежда", excel_data)
    assert len(result) == 1
    assert result[0]["Категория"] == "Одежда"


def test_transactions_by_user_choice_description_and_category(excel_data: list) -> None:
...
def test_transactions_by_user_choice_case_insensitive(excel_data: list) -> None:
    result = transactions_by_user_choice("ПРОДУКТЫ", excel_data)
    assert len(result) == 1
    assert result[0]["Описание"] == "Покупка продуктов"


def test_transactions_by_user_choice_not_found(excel_data: list) -> None:
