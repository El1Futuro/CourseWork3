...
def test_data_cards_in_period_empty() -> None:
    result = data_cards_in_period(pd.DataFrame())
    assert result.empty


def test_data_cards_in_period_one_transaction(transactions: DataFrame) -> None:
...
def test_data_cards_in_period_transactions_one_card(transactions: DataFrame) -> None:
    result = data_cards_in_period(transactions.iloc[:2])
    expected = {"Кэшбэк": {"6067": 3}, "Сумма расходов": {"6067": -300}}
    assert result[0].to_dict() == expected


def test_data_cards_in_period_transactions_cards(transactions: DataFrame) -> None:
